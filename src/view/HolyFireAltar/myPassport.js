...
const MyPassport = (props) => {
    ...
    const getMyFireSeed = async () => {
        const listLength = await handleSeedViewMethod("getOwnerIdlength", [])
        let list = []
        if (listLength <= 0) {
            return
        }
        for (let i = 0; i < listLength; i++) {
            const id = await handleSeedViewMethod("ownerOfId", [state.account, i])
            const balance = await handleSeedViewMethod("balanceOf", [state.account, id])
            list.push({
                id,
                balance,
            })
    ...
    return (
        <MyPassport>
            ...
            {/*            </div>*/}
            {/*            <div className="more-btn">*/}
            {/*                <a target="_blank" href={develop.openSeaNFT }>OpenSea</a>*/}
            {/*            </div>*/}
            {/*        </div>*/}
            {/*    </div>*/}
            {/*</div>*/}
